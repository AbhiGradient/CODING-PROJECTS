...
\begin{tikzpicture}[remember picture, overlay]
  % top band
  \fill[pine] (current page.north west) rectangle ([yshift=-5.6cm]current page.north east);
  \fill[brass] ([yshift=-5.6cm]current page.north west) rectangle ([yshift=-5.66cm]current page.north east);
  \fill[pine!70!black] ([yshift=-5.66cm]current page.north west) rectangle ([yshift=-5.74cm]current page.north east);
  % bottom band
  \fill[pine] (current page.south west) rectangle ([yshift=1.35cm]current page.south east);
  \fill[brass] ([yshift=1.35cm]current page.south west) rectangle ([yshift=1.41cm]current page.south east);
  % emblem: monogram roundel sitting on the top band
  \node[circle, draw=brass, line width=1.1pt, fill=ivory, minimum size=2.5cm] (emblem) at ([yshift=-2.55cm]current page.north) {};
  \node[align=center, font=\bfseries] at (emblem) {\color{pine}\fontsize{18}{18}\selectfont SC\\[-2pt]{\color{brass}\fontsize{7}{7}\selectfont BUS}};
  % corner accent rules framing the whole sheet
  \draw[brass, line width=0.6pt] ([xshift=1.1cm,yshift=-6.3cm]current page.north west) -- ([xshift=1.1cm,yshift=1.9cm]current page.south west);
  \draw[brass, line width=0.6pt] ([xshift=-1.1cm,yshift=-6.3cm]current page.north east) -- ([xshift=-1.1cm,yshift=1.9cm]current page.south east);
\end{tikzpicture}

\vspace*{3.35cm}
\begin{center}
{\color{white}\Large\textsc{Zeal College of Engineering and Research, Narhe, Pune}}\\[5pt]
...
\begin{center}
{\color{brass}\footnotesize\textsc{\hspace{2pt}Software Requirements Specification\hspace{2pt}}}\\[10pt]
{\color{pine}\fontsize{32}{38}\selectfont\bfseries Smart College\\[3pt] Bus Management System}

\vspace{0.55cm}
{\color{brass}\rule{3.4cm}{1.1pt}}\hspace{6pt}\raisebox{-2.4pt}{\textcolor{brass}{\footnotesize$\blacklozenge$}}\hspace{6pt}{\color{brass}\rule{3.4cm}{1.1pt}}

\vspace{0.75cm}
{\color{pinelight}\Large\itshape A Role-Based Web Platform for College Transport Management}

\vspace{1.5cm}
{\color{inktext}\large\bfseries Version 2.0} \quad {\color{softgray}\large\textbar} \quad {\color{inktext}\large \today}
\end{center}

\vspace{1.7cm}
\begin{center}
{\color{softgray}\large Prepared by \textbf{\color{pine}Team CampusTransit}}\\[3pt]
{\color{softgray}\small B.Tech (Artificial Intelligence \& Data Science Engineering), Second Year}

\vspace{0.55cm}
{\color{brass}\footnotesize\textsc{Eleven Contributing Members}}\\[7pt]
\begin{tabular}{p{5.3cm}p{5.3cm}}
{\color{inktext}\footnotesize 1.\ Om Hedawoo} & {\color{inktext}\footnotesize 7.\ Shubham Narsale} \\[3pt]
{\color{inktext}\footnotesize 2.\ Abhishek Sapkale} & {\color{inktext}\footnotesize 8.\ Prathamesh Shetty} \\[3pt]
{\color{inktext}\footnotesize 3.\ Sangraj Kadam} & {\color{inktext}\footnotesize 9.\ Soham Bandge} \\[3pt]
{\color{inktext}\footnotesize 4.\ Om Shahane} & {\color{inktext}\footnotesize 10.\ Saujas Salunke} \\[3pt]
{\color{inktext}\footnotesize 5.\ Mehul Umredkar} & {\color{inktext}\footnotesize 11.\ Soham Randhir} \\[3pt]
{\color{inktext}\footnotesize 6.\ Sarth Dange} & \\
\end{tabular}
\end{center}

\vfill
\begin{center}
{\color{white}\small Confidential \textbar\ Prepared for Academic Project Evaluation}
\end{center}
\vspace{0.55cm}
\restoregeometry
\end{titlepage}
...
The application is deployed as a cloud-hosted web service accessible via any modern browser (Chrome, Edge, Firefox) over HTTPS, requiring no client-side installation. The production environment runs Node.js on a managed cloud platform (Render) backed by a MySQL database instance hosted on Railway.

\section{Project Resources}
The complete source code, live deployment, and supporting infrastructure for the project are accessible at the following locations:

\begin{longtable}{@{}p{3.4cm}p{10.7cm}@{}}
\toprule
\rowcolor{pine!10}\textbf{Resource} & \textbf{Location} \\
\midrule
\endhead
GitHub Repository & \url{https://github.com/AbhiGradient/Bus-management-system} \\
Live Deployment & \url{https://bus-management-system-urmo.onrender.com/} \\
Application Hosting & Render (Node.js/Express web service) \\
Database Hosting & Railway (managed cloud MySQL instance) \\
\bottomrule
\end{longtable}
...
\begin{itemize}[leftmargin=1.3em]
  ...
  \item MySQL Server (relational database engine, hosted on Railway)
...
\section{Team Members Summary}
\begin{longtable}{@{}p{1.4cm}p{4cm}p{2.4cm}p{5.3cm}@{}}
\toprule
\rowcolor{pine!10}\textbf{Sr.\ No.} & \textbf{Name} & \textbf{Roll No.} & \textbf{Webpage Assigned} \\
\midrule
\endhead
1 & Om Hedawoo & AD1125 & Login \\
2 & Abhishek Sapkale & AD1156 & Admin Dashboard \\
3 & Sangraj Kadam & AD1155 & Student Dashboard \& Bus Details \\
4 & Om Shahane & AD1158 & Driver Dashboard \& Today's Route \\
5 & Mehul Umredkar & AD1137 & Trip Start \& Trip End \\
6 & Sarth Dange & AD1117 & QR Attendance \& Live GPS Tracking \\
7 & Shubham Narsale & AD1141 & Seat Management \\
8 & Prathamesh Shetty & AD1160 & Buses \& Assign Bus \\
9 & Soham Bandge & AD1109 & Fees \& Payments \\
10 & Saujas Salunke & AD1157 & Requests \\
11 & Soham Ranadhir & AD1149 & Reports \& Notifications \\
\bottomrule
\end{longtable}
